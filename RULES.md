...
the unit executes code. Identical bytes in `x.ts` and `x.d.ts` produce the same
findings and different profiles, and that difference is the whole rule.

**What this deliberately does NOT cover.** A generated file usually says so in a
header:

```
// Generated by <tool> by running `<command>` (hash: ...)
```

Demoting on that line was considered and refused. The header is text inside the
audited unit, which means the audited unit writes it. A payload that types one
...
name are already the weaker case, which is why §3.2 exists: that convention was
forgeable, a published attack forged it, and the fix was a second signal the
attacker does not control rather than a wider list of trusted directories.

If a generated-file signal is ever wanted, it has to come from outside the unit
— a lockfile, a build manifest, a registry the author does not write.
...
| `FSW-002` | CRITICAL | high | Modifies agent instructions or config (`~/.claude/`, `CLAUDE.md`, `AGENTS.md`, `settings.json`, `.mcp.json`, `~/.codex/config.toml`, `opencode.json`) | Rewrites the rules the agent follows — a persistent foothold in your assistant | Only as the unit's stated purpose. The redirect anchor depends on context: a code-suffix file (`.sh`/`.bash`/`.zsh`/`.fish`/`.ps1`/`.bat`/`.cmd`/`.py`/`.js`/`.mjs`/`.cjs`/`.ts`/`.rb`/`.pl`/`.php`/`.lua`/`.r`) or a markdown `bash`/`sh`/`shell`/`zsh`/`console` fence drops the HTML-tag exclusion entirely; a genuine `.html`/`.htm`/`.xhtml` document uses the strictest anchor instead (a `>` counts only at line start or after whitespace/a digit/`&`). That anchor's char class alone would let a tag close with a trailing space, `<p >`, misfire as a redirect there; `engine._scan_text` closes that gap for `.html`/`.htm`/`.xhtml` specifically by rejecting an anchor match that still sits inside an unclosed tag and continuing the search on the same line, so a genuine redirect later on the line is still caught. A real redirect with no space before `>` (`cat<p>CLAUDE.md`) still stays silent there — a documented gap. Everywhere else (markdown prose outside a shell-labeled fence, and any other file suffix — `.json`, `.txt`, `.yaml`, unrecognized) a `>` closing a BARE HTML open tag (`<` plus 1-8 letters, e.g. `<p>`) is excluded, but a `>` closing a tag WITH an attribute (`<p class="x">`) is not, because a quote — not a tag-name letter — sits right before it. Known gaps, both false-negative and false-positive, apply only in that last (non-code, non-`.html`) context: (1) the shell shape "command, input redirect from a one-word file, output redirect into a control file" with no spaces (`cat<p>CLAUDE.md`) still reads as an HTML tag and stays silent; (2) raw HTML with an attribute around a control filename (`<p class="x">AGENTS.md` in a `.md` file's body) can still fire |
...
- `scope_search` — one of six values, naming how the upward walk ended.
  ...
  **conclusive**: the search saw enough of the ancestor chain to trust its
  ...
  unexamined), `unreadable_ancestor` (an ancestor could not be read — a
  permission error, or it is not there), and `stopped_at_unit_marker` (a
  non-skill marker sat at the target itself) are **not**: the search stopped
  without ruling out a stronger marker further up.
  ...
  `stopped_at_unit_marker` is the same false clean at the very first step: a
  non-skill marker (`.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json`,
  `opencode.json`) always wins the climb and stops it immediately, because
  that marker is stronger than any `SKILL.md` beneath it. When that marker
  sits AT the scanned target — a claude plugin or opencode project root
  scanned directly — the climb stops after one iteration, `widened` is
  `false`, and nothing above the target was ever examined. A
  `marketplace.json` could enclose that exact plugin, but the break fires
  before the climb ever gets to look for it. Reporting `marker_at_target`
  there would claim a search that ran one step, not eight.
...
additive. When `scope_search` is `depth_limit`, `unreadable_ancestor`, or
`stopped_at_unit_marker`, `coverage_limits` carries one more entry saying so —
conditional, unlike the standing SKIP_DIRS limit below, because it depends on
how THIS unit's search went. The text report prints the same caveat next to
the scope line, so the human-readable report cannot say less than the JSON
does.
