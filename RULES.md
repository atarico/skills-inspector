...
| `FSW-002` | CRITICAL | high | Modifies agent instructions or config (`~/.claude/`, `CLAUDE.md`, `AGENTS.md`, `settings.json`, `.mcp.json`, `~/.codex/config.toml`, `opencode.json`) | Rewrites the rules the agent follows — a persistent foothold in your assistant | Only as the unit's stated purpose. Known gap: in markdown prose outside a shell-labeled fence, a `>` that closes what reads as an HTML open tag (a `<` plus 1-8 letters) is not treated as a redirect, so the shell shape "command, input redirect from a one-word file, output redirect into a control file" with no spaces stays silent there. In code files and in `bash`/`sh`/`shell`/`zsh`/`console` fences, the tag exclusion does not apply |
