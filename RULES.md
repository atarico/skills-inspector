...
| `FSW-002` | CRITICAL | high | Modifies agent instructions or config (`~/.claude/`, `CLAUDE.md`, `AGENTS.md`, `settings.json`, `.mcp.json`, `~/.codex/config.toml`, `opencode.json`) | Rewrites the rules the agent follows — a persistent foothold in your assistant | Only as the unit's stated purpose. The redirect anchor depends on context: in code files and in `bash`/`sh`/`shell`/`zsh`/`console` fences the HTML-tag exclusion does not apply at all; in a genuine `.html`/`.htm`/`.xhtml` document the strictest anchor applies (a `>` counts only at line start or after whitespace/a digit/`&`); everywhere else (markdown prose, any other file) a `>` closing a BARE HTML open tag (`<` plus 1-8 letters, e.g. `<p>`) is excluded, but a `>` closing a tag WITH an attribute (`<p class="x">`) is not, because a quote — not a tag-name letter — sits right before it. Known gaps, both false-negative and false-positive: (1) outside a shell-labeled fence or a `.html` file, the shell shape "command, input redirect from a one-word file, output redirect into a control file" with no spaces (`cat<p>CLAUDE.md`) still reads as an HTML tag and stays silent; (2) outside a `.html` file, raw HTML with an attribute around a control filename (`<p class="x">AGENTS.md` in a `.md` file's body) can still fire |
