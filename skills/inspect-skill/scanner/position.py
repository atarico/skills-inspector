...
#
# This is a LANGUAGE fact, not a location or invocation one: unlike the
# sample-directory convention above, nothing can outrank it. A payload cannot
# earn its way back to `active` by being referenced from an entry point, or by
# matching `auto_executed`'s discovery conventions — a toolchain does not
# execute a `.d.ts` either, for the same structural reason a human editing it
# cannot make TypeScript treat it as a value.
#
# Deliberately keyed on the suffix alone, never on a header inside the file
# ("// Generated by ..."). A header is attacker-controlled text — a payload
# that types that line would buy its own demotion — while the suffix is a
# fact about what the TOOLCHAIN does with the file, which the file's own
# content cannot change (D2, same document).
def is_declaration_file(relpath: str) -> bool:
    """Is this a TypeScript ambient declaration file?

    A pure suffix test on the FULL filename — `str.endswith`, not
    `PurePosixPath.suffix` — because `.suffix` on "worker.d.ts" is only
    `.ts`; the `.d` that actually marks the file as declaration-only lives one
    suffix further back, which `PurePosixPath.suffixes` also gets right but at
    the cost of splitting on every dot, including ones that appear inside a
    version-numbered filename with no bearing on this question.
    """
    return PurePosixPath(relpath).name.lower().endswith(".d.ts")
...
def file_base_position(relpath: str, invoked: bool = False) -> str:
    ...
    `is_declaration_file` is checked before either override gets a say, and it
    stays DOCUMENTARY regardless of them — see its own comment for why nothing
    can outrank a fact about what the TypeScript toolchain does with the file.
    """
    if is_declaration_file(relpath):
        return DOCUMENTARY
